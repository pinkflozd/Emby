define(["datetime", "jQuery", "dom", "humanedate", "cardStyle", "listViewStyle"], function(datetime, $, dom) {
    ...
    function onEditServerNameClick(e) {
        ...
        return require(["prompt"], function(prompt) {
            prompt({
                ...
                Dashboard.showLoadingMsg(), ApiClient.getServerConfiguration().then(function(config) {
                    config.ServerName = value, ApiClient.updateServerConfiguration(config).then(function() {
                        page.querySelector(".serverNameHeader").innerHTML = value, Dashboard.hideLoadingMsg()
                    })
                })
            })
        }), e.preventDefault(), !1
    ...
    window.DashboardPage = {
            ...
            renderInfo: function(page, sessions, forceUpdate) {
                DashboardPage.renderActiveConnections(page, sessions), DashboardPage.renderPluginUpdateInfo(page, forceUpdate), Dashboard.hideLoadingMsg()
            },
            ...
            installPluginUpdate: function(button) {
                ...
                Dashboard.showLoadingMsg(), ApiClient.installPlugin(name, guid, classification, version).then(function() {
                    Dashboard.hideLoadingMsg()
                })
            },
            updateApplication: function() {
                var page = $.mobile.activePage;
                $("#btnUpdateApplication", page).buttonEnabled(!1), Dashboard.showLoadingMsg(), ApiClient.getScheduledTasks().then(function(tasks) {
                    var task = tasks.filter(function(t) {
                        return t.Key == DashboardPage.systemUpdateTaskKey
                    })[0];
                    ApiClient.startScheduledTask(task.Id).then(function() {
                        DashboardPage.pollForInfo(page), Dashboard.hideLoadingMsg()
                    })
                })
            },
